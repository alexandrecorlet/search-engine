...
class InvertedIndex():
    ...
    def get_top_k(self, query, k=5, k1=1.5, b=0.75):
        ...
        for token in preprocessed_query:
            if token in self.index:
                ...
                for doc_id, tf in self.index[token].items():
                    num = tf * (k1 + 1)
                    den = tf + self.k1 * ((1 - self.b + self.b) / (self.doc_lengths[doc_id] / self.avg_doc_length))
        ...
        top_k_docs = [doc_id for doc_id, _ in sorted_scores[:top_l]]
        # Return top K docs :D
        return top_k_docs
